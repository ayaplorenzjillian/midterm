...
class User{
	...
	public function new_user($email,$password,$lastname,$firstname, $access){
		...
		$data = [
			[$email,$password,$lastname,$firstname,$NOW,$NOW,'1', $access],
		];
		$stmt = $this->conn->prepare("INSERT INTO tbl_users (user_lastname, user_firstname, user_email, user_password, user_access) VALUES (?,?,?,?,)");
		try {
